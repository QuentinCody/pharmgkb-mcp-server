...
interface QueryEnv {
    PHARMGKB_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv) {
    const handler = createQueryDataHandler("PHARMGKB_DATA_DO", "pharmgkb");

    server.registerTool(
        "pharmgkb_query_data",
        {
            title: "Query Staged PharmGKB Data",
            description:
                "Query staged PharmGKB data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
                data_access_id: z
                    .string()
                    .min(1)
